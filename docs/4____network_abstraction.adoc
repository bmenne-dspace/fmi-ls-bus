...
The operation has three arguments _Arg1_ (1 byte long), _Arg2_Length_ (1 byte long), _Arg2_Data_ (_<Arg2Length>_ bytes long), whereby argument 2 and argument 3 form a coherent variable length argument.
...
h|Name
3+|OperationName
.6+h|Content h|Argument h|Size h|Value
|OP Code
|4 bytes
|0x01
...
|Length
|4 bytes
|4 + 4 + 1 + 1 + <Arg2_Length> = 10 + <Arg2_Length>

|Arg1
|1 byte
|Contains the value of the first operation specific argument.

|Arg2_Length
|1 byte
|Contains the actual length of the value of the second argument.

|Arg2_Data
|<Arg2_Length> bytes
|Contains the value of the second operation specific argument.
...
The input Clocks (`Rx_Clock`) shall be `triggered` Clocks if they are to be driven by the respective Clock of the sending FMU or Bus Simulation.

Alternatively, the input Clocks (`Rx_Clock`) can be `periodic` Clocks.
In this case the `Rx_Clock` of the receiving FMU needs to match the relevant Clock of the sending FMU, which causes them to be scheduled for the same time instances or the Bus Simulation shall buffer the accumulated Bus Operations for the receiving FMUs and deliver them in aggregated form so that no Bus Operation is lost.

NOTE: This may allow for more efficient simulation in the case that there exists a more or less static communications schedule and more detailed simulation of timing variation is not needed.
...
|application/org.fmi-standard.fmi-ls-bus.lin; version="1.0.0-beta.1"
|Binary variables simulating a LIN network

|application/org.fmi-standard.fmi-ls-bus.flexray; version="1.0.0-beta.3"
|Binary variables simulating a FlexRay network

|application/org.fmi-standard.fmi-ls-bus.ethernet; version="1.0.0-beta.1"
